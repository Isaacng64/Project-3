...
export default class AudioPlayer extends LightningElement {
  ...
  tickCallback(tickVolume, autoNotes) {
    ...
    for (let i = 0; i < autoNotes.length; i++) {
      ...
      this.autoPlaying.push(
        this.clientNotesAuto["piano"][autoNotes[i].octave][autoNotes[i].name]
      );
    }
  }
  ...
  play(playerNote, instrument) {
    if (playerNote.valid) {
      this.clientNotesAuto[instrument][playerNote.octave][
        playerNote.name
      ].play(); // can specify duration in player.play() function!
    } else {
      throw "no valid note to play!";
    }

    this.currentlyPlaying.push(
      this.clientNotesAuto[instrument][playerNote.octave][playerNote.name]
    );
  }
